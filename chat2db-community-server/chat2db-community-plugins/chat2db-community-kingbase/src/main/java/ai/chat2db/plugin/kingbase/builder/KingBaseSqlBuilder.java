...
import static ai.chat2db.plugin.kingbase.KingBaseSqlEscapes.escapeSqlLiteral;
import static ai.chat2db.plugin.kingbase.KingBaseSqlEscapes.requireSafeExpression;
public class KingBaseSqlBuilder extends DefaultSqlBuilder {
    ...
    public String buildCreateTable(Table table, TableBuilderConfig tableBuilderConfig) {
        ...
        script.append(KingBaseSqlEscapes.quoteIdentifier(table.getName())).append(SQLConstants.SPACE_OPEN_PARENTHESIS).append(SQLConstants.SPACE).append(SQLConstants.LINE_SEPARATOR);
        ...
        if(StringUtils.isNotBlank(table.getTablespace())){
            script.append(" TABLESPACE ").append(KingBaseSqlEscapes.quoteIdentifier(table.getTablespace())).append(SQLConstants.SEMICOLON);
        }else {
        ...
        if (StringUtils.isNotBlank(table.getComment())) {
            script.append(SQLConstants.LINE_SEPARATOR);
            script.append(SQL_COMMENT_TABLE).append(SQLConstants.SPACE).append(KingBaseSqlEscapes.quoteIdentifier(table.getName())).append(SQLConstants.SQL_IS_SINGLE_QUOTE)
                    .append(escapeSqlLiteral(table.getComment())).append(SQLConstants.SINGLE_QUOTE_SEMICOLON_LINE_SEPARATOR);
        }
    ...
    public String buildAlterTable(Table oldTable, Table newTable) {
        ...
        if (!StringUtils.equalsIgnoreCase(oldTable.getName(), newTable.getName())) {
            script.append(SQL_ALTER_TABLE).append(KingBaseSqlEscapes.quoteIdentifier(oldTable.getName()));
            script.append(SQLConstants.TAB).append(SQL_RENAME).append(KingBaseSqlEscapes.quoteIdentifier(newTable.getName())).append(SQLConstants.SEMICOLON_LINE_SEPARATOR);
        ...
        for (TableColumn tableColumn : columnNameList) {
            script.append(SQL_ALTER_TABLE).append(KingBaseSqlEscapes.quoteIdentifier(newTable.getName())).append(SQLConstants.SPACE).append("RENAME COLUMN ")
                    .append(KingBaseSqlEscapes.quoteIdentifier(tableColumn.getOldName())).append(" TO ").append(KingBaseSqlEscapes.quoteIdentifier(tableColumn.getName())).append(SQLConstants.SEMICOLON_LINE_SEPARATOR);
        }
        ...
        scriptModify.append(SQL_ALTER_TABLE).append(KingBaseSqlEscapes.quoteIdentifier(newTable.getName())).append(" \n");
        ...
        if (!StringUtils.equals(oldTable.getComment(), newTable.getComment())) {
            script.append(SQLConstants.LINE_SEPARATOR);
            script.append(SQL_COMMENT_TABLE).append(SQLConstants.SPACE).append(KingBaseSqlEscapes.quoteIdentifier(newTable.getName())).append(SQLConstants.SQL_IS_SINGLE_QUOTE)
                    .append(escapeSqlLiteral(newTable.getComment())).append(SQLConstants.SINGLE_QUOTE_SEMICOLON_LINE_SEPARATOR);
        }
        for (TableColumn tableColumn : newTable.getColumnList()) {
            KingBaseColumnTypeEnum typeEnum = KingBaseColumnTypeEnum.getByType(tableColumn.getColumnType());
            if(typeEnum == null){
    ...
    public String buildCreateDatabase(Database database) {
        ...
        sqlBuilder.append(SQL_CREATE_DATABASE+KingBaseSqlEscapes.quoteIdentifier(database.getName()));
        String owner = database.getOwner();
        if (StringUtils.isBlank(owner)) {
            owner = SYSTEM_KEYWORD;
        }
        sqlBuilder.append(" WITH  OWNER = ").append(KingBaseSqlEscapes.quoteIdentifier(owner));
        if (StringUtils.isNotBlank(database.getCharset())) {
            sqlBuilder.append(SQL_ENCODING).append(requireSafeExpression(database.getCharset(), "database charset")).append(SQLConstants.EMPTY);
        }
        sqlBuilder.append(SQLConstants.SEMICOLON_LINE_SEPARATOR);

        if (StringUtils.isNotBlank(database.getComment())) {
            sqlBuilder.append(SQL_COMMENT_DATABASE).append(KingBaseSqlEscapes.quoteIdentifier(database.getName())).append(SQLConstants.SQL_IS_SINGLE_QUOTE).append(escapeSqlLiteral(database.getComment())).append(SQLConstants.SINGLE_QUOTE_SEMICOLON);
        }
    ...
    public String buildCreateSchema(Schema schema){
        StringBuilder sqlBuilder = new StringBuilder();
        sqlBuilder.append(SQL_CREATE_SCHEMA+KingBaseSqlEscapes.quoteIdentifier(schema.getName())+SQLConstants.EMPTY);
        String owner = schema.getOwner();
        if(StringUtils.isBlank(schema.getOwner())){
            owner = SYSTEM_KEYWORD;
        }
        sqlBuilder.append(" AUTHORIZATION ").append(KingBaseSqlEscapes.quoteIdentifier(owner));
        return sqlBuilder.toString();
    }
