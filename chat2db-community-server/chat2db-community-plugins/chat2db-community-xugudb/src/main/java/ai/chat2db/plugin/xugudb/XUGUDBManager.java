...
public class XUGUDBManager extends DefaultDBManager implements IDbManager {
    ...
    private void exportTables(Connection connection, String schemaName, AsyncContext asyncContext) throws SQLException {
        String sql = String.format(SQL_SELECT_TABLE_NAME_ALL_TABLES, XugudbSqlEscapes.escapeSqlLiteral(schemaName));
    ...
    private void exportTable(Connection connection, String tableName, String schemaName, AsyncContext asyncContext) throws SQLException {
        String sql = """
                ...
                FROM dual;
                """;
        try (PreparedStatement statement = connection.prepareStatement(String.format(sql, XugudbSqlEscapes.escapeSqlLiteral(tableName), XugudbSqlEscapes.escapeSqlLiteral(tableName), XugudbSqlEscapes.escapeSqlLiteral(schemaName))); ResultSet resultSet = statement.executeQuery()) {
            ...
            if (resultSet.next()) {
                ...
                if (StringUtils.isNotBlank(comment)) {
                    sqlBuilder.append(SQL_COMMENT_TABLE).append(formatSchemaName).append(".").append(formatTableName)
                            .append(" IS ").append("'").append(XugudbSqlEscapes.escapeSqlLiteral(comment)).append("';");
                }
    ...
    private void exportTableColumnComment(Connection connection, String schemaName, String tableName, AsyncContext asyncContext) throws SQLException {
        String sql = String.format(SQL_SELECT_COLNAME_COMMENT_SYS_SYSCOLUMNCOMMENTS +
                "where SCHNAME = '%s' and TVNAME = '%s'and TABLE_TYPE = 'TABLE';", XugudbSqlEscapes.escapeSqlLiteral(schemaName), XugudbSqlEscapes.escapeSqlLiteral(tableName));
        try (PreparedStatement preparedStatement = connection.prepareStatement(sql); ResultSet resultSet = preparedStatement.executeQuery()) {
            while (resultSet.next()) {
                ...
                sqlBuilder.append(SQL_COMMENT_COLUMN).append(format(schemaName)).append(".").append(format(tableName))
                        .append(".").append(format(columnName)).append(" IS ").append("'").append(XugudbSqlEscapes.escapeSqlLiteral(comment)).append("';").append("\n");
                asyncContext.write(sqlBuilder.toString());
            }
        }
    }
    ...
    private void exportView(Connection connection, String viewName, String schemaName, AsyncContext asyncContext) throws SQLException {
        String sql = String.format(SQL_SELECT_DBMS_METADATA_GET_DDL, XugudbSqlEscapes.escapeSqlLiteral(viewName), XugudbSqlEscapes.escapeSqlLiteral(schemaName));
        try (PreparedStatement preparedStatement = connection.prepareStatement(sql); ResultSet resultSet = preparedStatement.executeQuery()) {
            if (resultSet.next()) {
                StringBuilder sqlBuilder = new StringBuilder();
    ...
    private void exportProcedure(Connection connection, String schemaName, String procedureName, AsyncContext asyncContext) throws SQLException {
        String sql = String.format(ROUTINES_SQL, "PROC", XugudbSqlEscapes.escapeSqlLiteral(schemaName), XugudbSqlEscapes.escapeSqlLiteral(procedureName));
    ...
    private void exportTriggers(Connection connection, String schemaName, AsyncContext asyncContext) throws SQLException {
        String sql = String.format(TRIGGER_SQL_LIST, XugudbSqlEscapes.escapeSqlLiteral(schemaName));
        try (PreparedStatement preparedStatement = connection.prepareStatement(sql); ResultSet resultSet = preparedStatement.executeQuery()) {
            while (resultSet.next()) {
    ...
    private void exportTrigger(Connection connection, String schemaName, String triggerName, AsyncContext asyncContext) throws SQLException {
        String sql = String.format(TRIGGER_SQL, XugudbSqlEscapes.escapeSqlLiteral(schemaName), XugudbSqlEscapes.escapeSqlLiteral(triggerName));
        try (PreparedStatement preparedStatement = connection.prepareStatement(sql); ResultSet resultSet = preparedStatement.executeQuery()) {
            if (resultSet.next()) {
                StringBuilder sqlBuilder = new StringBuilder();
